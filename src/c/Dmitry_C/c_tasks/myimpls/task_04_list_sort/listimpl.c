...
bool list_is_sorted(list_t *list, list_comparator *comp) {
  if (list_is_empty(list)) {
    return true;
  }
  struct ListEntry *current_entry = list->head;
  ...
  while (next_entry != NULL) {
    if (comp(current_entry, next_entry, NULL) > 0) {
      return false;
    }
  }
  return true;
...
void list_print(list_t *list, list_entry_printer *print_entry) {
  printf("[");
  if (!list_is_empty(list)) {
...
void int_printer (const struct ListEntry *entry, void *aux) {
  // TODO;
}
