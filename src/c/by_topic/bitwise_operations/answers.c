...
int get_digit_at(int row, int column) {
    int is_second_nibble = (1 + column) & 1;
    int byte_index = 5 * row + (column + 1) / 2;
    if (is_second_nibble) {
        return sudoku_field_bits[byte_index] & 0xf;
    }
    return sudoku_field_bits[byte_index] >> 4;
}
...
void set_digit_at(int row, int column, int value) {
    int is_second_nibble = (1 + column) & 1;
    int byte_index = 5 * row + (column + 1) / 2;
    int shift = 4 * !is_second_nibble;
    sudoku_field_bits[byte_index] = sudoku_field_bits[byte_index] & (0xf << (4 - shift)) | (value << shift);
}
...
void display_field() {
    for (int i = 0; i < 9; i++) {
        ...
        for (int j = 0; j < 9; j++) {
            ...
            int digit = get_digit_at(i, j);
            char cell = digit == 0 ? '.' : digit + '0';
            printf("%c ", cell);
        }
        printf("|\n");
    }
