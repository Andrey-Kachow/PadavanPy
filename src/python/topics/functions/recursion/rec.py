...
'''
	...
	Функция уже частично написана. Нужно заполнить пропуски,
	то есть LIST_SOMETHING и LIST_SOMETHING_ELSE
'''
def rec_all(values):
	if BOOL_SOMETHING:
		return True
	else:
		return bool(head(values)) and LIST_SOMETHING_ELSE
...
'''
	...
	Функция уже частично написана. Нужно заполнить пропуски,
	то есть LIST_SOMETHING и LIST_SOMETHING_ELSE, и т.п.
'''
def rec_take(amount, values):
	if base_case(values) or INT_SOMETHING == 0:
		return LIST_SOMETHING
	else:
		return cons(head(values), LIST_SOMETHING_ELSE)
