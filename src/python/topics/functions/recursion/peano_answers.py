...
def peano_add(n, m):
	if n == 0:
		...
		return succ(peano_add(pred(n), m))
...
def peano_sub(n, m):
	if m == 0:
		...
		return pred(peano_sub(n, pred(m)))
...
def peano_mul(n, m):
	if n == 0:
		...
		return peano_add(peano_mul(pred(n), m), m)
...
def peano_div(n, m):
	...
	if n == 0 or n < m:
		...
		return succ(peano_div(peano_sub(n, m), m))
...
def peano_mod(n, m):
	if m == 0:
		raise ZeroDivisionError("На ноль делить нельзя!")
	return peano_sub(n, peano_mul(m, peano_div(n, m)))
...
def peano_pow(n, m):
	if m == 0:
		...
		return peano_mul(n, peano_pow(n, pred(m)))
